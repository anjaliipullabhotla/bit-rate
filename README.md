...
Since, increasing N only results in a logarithmic increase in bit rate, whereas increasing S_c results in a linear increase in bit rate, the task required finding an optimal N such that plenty of information is encoded in a single selection without compromising accuracy/reaction time. We removed sequences with duplicate letters since this can create issues with keystroke recognition.
...
| **26 keys, len 2**     | **650** | **9.34** | **Medium complexity/high accuracy, high information transmission, highest performance** |
